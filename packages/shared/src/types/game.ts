...
export type ServerMessage =
  ...
  | { event: 'countdown_tick'; data: { countdown: number } }
  | { event: 'round_flying'; data: Record<string, never> }
  ...
  | { event: 'bet_confirmed'; data: { panelId: PanelId; betId: string; balance: number } }
  | { event: 'cashout_success'; data: { panelId: PanelId; multiplier: number; payout: number; balance: number } }
  ...
  | { event: 'registered'; data: { playerId: string } }
  | { event: 'error'; data: { code: string; message: string } }

/** Client → Server WebSocket messages */
export type ClientMessage =
  | { action: 'register'; data: { playerId?: string } }
  | { action: 'place_bet'; data: { panelId: PanelId; betAmount: number; autoCashout?: number; lobbyToken?: string; lobbySessionId?: string } }
  | { action: 'cashout'; data: { panelId: PanelId; sessionId: string; lobbyToken?: string; lobbySessionId?: string } }
