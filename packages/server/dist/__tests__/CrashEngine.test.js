...
describe('CrashEngine', () => {
    ...
    describe('placeBet', () => {
        ...
        it('rejects bet below MIN_AMOUNT', async () => {
            startAndAdvanceToBetting();
            const playerId = uniquePlayer();
            await expect(engine.placeBet({ playerId, panelId: 'A', betAmount: 0.5 })).rejects.toThrow(GameError);
        });
        it('rejects autoCashout below MIN_AUTO_CASHOUT (1.01)', async () => {
            startAndAdvanceToBetting();
            const playerId = uniquePlayer();
            await expect(engine.placeBet({ playerId, panelId: 'A', betAmount: 100, autoCashout: 1.0 })).rejects.toThrow(GameError);
        });
        it('rejects when insufficient balance', async () => {
            startAndAdvanceToBetting();
            const playerId = uniquePlayer();
            await expect(engine.placeBet({ playerId, panelId: 'A', betAmount: 999999 })).rejects.toThrow(GameError);
        });
    ...
    describe('cashout', () => {
        ...
        it('rejects in non-FLYING phase', async () => {
            ...
            await expect(engine.cashout({ playerId, panelId: 'A', sessionId: betResult.sessionId })).rejects.toThrow(GameError);
        });
        it('rejects with wrong sessionId', async () => {
            ...
            await engine.placeBet({ playerId, panelId: 'A', betAmount: 100 });
            vi.advanceTimersByTime(8001); // → FLYING
            await expect(engine.cashout({ playerId, panelId: 'A', sessionId: 'wrong-session' })).rejects.toThrow(GameError);
        });
        ...
        it('rejects when no bet exists', async () => {
            ...
            const betResult = await engine.placeBet({ playerId, panelId: 'A', betAmount: 100 });
            vi.advanceTimersByTime(8001); // → FLYING
            await expect(engine.cashout({ playerId, panelId: 'B', sessionId: betResult.sessionId })).rejects.toThrow(GameError);
        });
...
//# sourceMappingURL=CrashEngine.test.js.map