...
describe('API Integration (via CrashEngine)', () => {
    ...
    describe('phase guards', () => {
        it('rejects bet in WAITING phase', async () => {
            engine.start();
            const playerId = uniquePlayer();
            await expect(engine.placeBet({ playerId, panelId: 'A', betAmount: 100 })).rejects.toThrow(GameError);
        });
        ...
        it('rejects cashout in BETTING phase', async () => {
            ...
            const playerId = uniquePlayer();
            const bet = await engine.placeBet({ playerId, panelId: 'A', betAmount: 100 });
            await expect(engine.cashout({ playerId, panelId: 'A', sessionId: bet.sessionId })).rejects.toThrow(GameError);
        });
        it('rejects cashout in WAITING phase', async () => {
            engine.start();
            const playerId = uniquePlayer();
            await expect(engine.cashout({ playerId, panelId: 'A', sessionId: 'fake' })).rejects.toThrow(GameError);
        });
    });
    ...
    describe('validation middleware', () => {
        ...
        describe('validatePlaceBet', () => {
            it('passes for valid input', () => {
                const next = vi.fn();
                validatePlaceBet(mockReq({ playerId: 'p1', panelId: 'A', betAmount: 100 }), mockRes(), next);
                expect(next).toHaveBeenCalledWith();
            });
            it('rejects missing playerId', () => {
                const next = vi.fn();
                validatePlaceBet(mockReq({ panelId: 'A', betAmount: 100 }), mockRes(), next);
                expect(next).toHaveBeenCalledWith(expect.any(GameError));
            });
            it('rejects invalid panelId', () => {
                const next = vi.fn();
                validatePlaceBet(mockReq({ playerId: 'p1', panelId: 'X', betAmount: 100 }), mockRes(), next);
                expect(next).toHaveBeenCalledWith(expect.any(GameError));
            });
            it('rejects non-numeric betAmount', () => {
                const next = vi.fn();
                validatePlaceBet(mockReq({ playerId: 'p1', panelId: 'A', betAmount: 'abc' }), mockRes(), next);
                expect(next).toHaveBeenCalledWith(expect.any(GameError));
            });
            it('rejects autoCashout below MIN_AUTO_CASHOUT', () => {
                const next = vi.fn();
                validatePlaceBet(mockReq({ playerId: 'p1', panelId: 'A', betAmount: 100, autoCashout: 0.5 }), mockRes(), next);
                expect(next).toHaveBeenCalledWith(expect.any(GameError));
            });
        });
        describe('validateCashout', () => {
            it('passes for valid input', () => {
                const next = vi.fn();
                validateCashout(mockReq({ playerId: 'p1', panelId: 'A', sessionId: 'S-1' }), mockRes(), next);
                expect(next).toHaveBeenCalledWith();
            });
            it('rejects missing sessionId', () => {
                const next = vi.fn();
                validateCashout(mockReq({ playerId: 'p1', panelId: 'A' }), mockRes(), next);
                expect(next).toHaveBeenCalledWith(expect.any(GameError));
            });
        });
        describe('validateVerify', () => {
            it('passes for valid query params', () => {
                const next = vi.fn();
                validateVerify(mockReq({}, { serverSeed: 'seed', clientSeed: 'client', nonce: '1' }), mockRes(), next);
                expect(next).toHaveBeenCalledWith();
            });
            it('rejects missing serverSeed', () => {
                const next = vi.fn();
                validateVerify(mockReq({}, { clientSeed: 'client', nonce: '1' }), mockRes(), next);
                expect(next).toHaveBeenCalledWith(expect.any(GameError));
            });
            it('rejects non-numeric nonce', () => {
                const next = vi.fn();
                validateVerify(mockReq({}, { serverSeed: 'seed', clientSeed: 'client', nonce: 'abc' }), mockRes(), next);
                expect(next).toHaveBeenCalledWith(expect.any(GameError));
            });
        });
    });
