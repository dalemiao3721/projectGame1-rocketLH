...
export const BetRecord = {
    async insert(params) {
        const pool = getPool();
        await pool.query(`INSERT INTO rocketlh_bet_records
    ...
    async updateStatus(betId, status) {
        const pool = getPool();
        await pool.query(`UPDATE rocketlh_bet_records SET status = $1 WHERE bet_id = $2`, [status, betId]);
    },
};
//# sourceMappingURL=BetRecord.js.map