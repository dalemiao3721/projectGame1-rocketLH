...
import type { PanelBet } from './SessionStore.js'
...
export interface CrashEngineEvents {
  ...
  history_update: [recent: { sessionId: string; crashMultiplier: number }[]]
  settled: []
}
...
function getStandaloneBalance(playerId: string): number {
...
function adjustStandaloneBalance(playerId: string, amount: number): number {
...
function dbWrite(label: string, fn: () => Promise<void>): void {
  fn().catch((err) => {
    console.error(`[RocketLH DB] ${label} failed:`, err.message)
  })
}

export class CrashEngine extends EventEmitter<CrashEngineEvents> {
  private static instance: CrashEngine | null = null
  ...
  private history: { sessionId: string; crashMultiplier: number; crashedAt: string }[] = []
  ...
  async placeBet(params: {
    ...
    if (useLobby) {
      ...
      try {
        ...
        // Lobby settle failed (e.g. session expired) — fallback to in-memory
        console.warn(`[RocketLH] Lobby settle failed, falling back to standalone: ${err.message}`)
    ...
    dbWrite(`BetRecord.insert(${betId})`, () =>
      BetRecord.insert({
        betId,
        sessionId: round.sessionId,
        playerId,
  ...
  async cashout(params: {
    ...
    if (bet.lobbyToken && bet.lobbySessionId) {
      ...
      try {
        ...
        console.warn(`[RocketLH] Cashout lobby settle failed, falling back to standalone: ${err.message}`)
    ...
    // Persist to DB (fire-and-forget)
    dbWrite(`BetRecord.updateStatus(${bet.betId})`, () =>
      BetRecord.updateStatus(bet.betId, 'cashed_out')
    )
    const settlementId = `SETTLE-${Date.now()}-${panelId}`
    dbWrite(`Settlement.insert(${settlementId})`, () =>
      Settlement.insert({
        settlementId,
        betId: bet.betId,
        outcome: 'win',
  ...
  verifyFairness(params: {
    ...
    const { computedHash, computedCrashPoint } = this.provablyFair.verify(
      serverSeed, clientSeed, nonce,
      serverConfig.rtpSetting, serverConfig.volatilityLevel
    )
    return {
      isValid: true,
      computedHash,
  ...
  /** Access session store (for WS handlers) */
  getSessionStore(): SessionStore {
    return this.sessionStore
  }

  /** Get current round state (for reconnection) */
  getRoundState(): RoundStateResponse & { countdown: number } {
    const round = this.sessionStore.getCurrentRound()
    return {
      ...
      countdown: this.roundManager.getCountdown(),
    }
  }

  /** Get recent crash history */
  getHistory(limit: number = 10): { sessionId: string; crashMultiplier: number; crashedAt: string }[] {
    return this.history.slice(0, limit)
  }

  /** Get player balance (standalone mode or lobby mode) */
  async getPlayerBalance(playerId: string, lobbyToken?: string): Promise<number> {
    if (lobbyToken) {
  ...
  private startNextRound(): void {
    ...
    dbWrite(`CrashLog.insert(${drawId})`, () =>
      CrashLog.insert({
        drawId,
        sessionId,
  ...
  private bindRoundEvents(): void {
    ...
    this.roundManager.on('countdown_tick', (countdown) => {
      this.emit('countdown_tick', countdown)
    })
  ...
  private processAutoCashouts(currentMultiplier: number): void {
    ...
    for (const bet of activeBets) {
      if (
        ...
        dbWrite(`Settlement.insert(${autoSettleId})`, () =>
          Settlement.insert({
  ...
  private handleCrash(crashMultiplier: number): void {
    const round = this.sessionStore.getCurrentRound()
    if (!round) return
    ...
    for (const bet of activeBets) {
      ...
      dbWrite(`BetRecord.updateStatus(${bet.betId})`, () =>
        BetRecord.updateStatus(bet.betId, 'lost')
      )
      const lostSettleId = `SETTLE-LOST-${Date.now()}-${bet.panelId}`
      dbWrite(`Settlement.insert(${lostSettleId})`, () =>
        Settlement.insert({
    ...
    dbWrite(`CrashLog.updateCrashedAt(${round.sessionId})`, () =>
      CrashLog.updateCrashedAt(round.sessionId, crashedAt)
    )
