...
const VOLATILITY_PARAMS: Record<number, { alpha: number; beta: number }> = {
...
export class CrashMath {
  ...
  survivalProbability(targetMultiplier: number, rtp: number, volatility: number): number {
    ...
    if (rawP <= 0) return 1.0
    if (rawP >= 1) return 0.0
    const params = VOLATILITY_PARAMS[volatility]
    if (!params) return 0
