...
const RTP_VALUES = [94, 95, 96, 97, 98, 99] as const
const VOLATILITY_LEVELS = [1, 2, 3, 4, 5] as const
...
describe('Survival probability consistency (simulated vs analytical)', () => {
  ...
  for (const rtp of [94, 97, 99] as const) {
...
describe('Crash point distribution per volatility', () => {
  ...
  function getDistribution(vol: number) {
    const buckets = {
      under_1_5x: 0,
      '1_5x_to_2x': 0,
      '2x_to_5x': 0,
    ...
    for (let i = 0; i < ROUNDS; i++) {
      ...
      if (cp < 1.5) buckets.under_1_5x++
      else if (cp < 2) buckets['1_5x_to_2x']++
      else if (cp < 5) buckets['2x_to_5x']++
      else if (cp < 10) buckets['5x_to_10x']++
      else if (cp < 50) buckets['10x_to_50x']++
      else buckets.over_50x++
    }

    return Object.fromEntries(
      Object.entries(buckets).map(([k, v]) => [k, v / ROUNDS])
    )
  }
  ...
  it('volatility ordering: lower vol level → more low crash points', () => {
    ...
    for (let i = 0; i < dists.length - 1; i++) {
      expect(dists[i].dist.under_1_5x).toBeGreaterThanOrEqual(
        dists[i + 1].dist.under_1_5x - 0.03 // small tolerance
      )
    }
  })
})
